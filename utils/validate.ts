const validatePassword = (password: string) => {
  if (password) {
    const patternCnt = [{ type: /^(?=.*?[a-zA-Z])/ }, { type: /^(?=.*?[0-9]).{8,16}$/ }, { type: /^(?=.*?[#?!@$%^&*-])/ }].filter(
      (item) => {
...
const validateEmail = (email: string) => {
  if (email)
    return !/^[a-zA-Z0-9+-_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/.test(email) ? '이메일 형식에 맞지않습니다. 다시 입력해주세요.' : null;
  return null;
};

const validateCount = (count: number) => {
  if (count) {
    if (count < 0) return '갯수가 0보다 작을 수 없습니다.';
    if (count > 10) return '10개이상 구매가 불가능합니다.';
  }
  return null;
};
...
const validateNick = (nick: string) => {
  if (nick) {
    const patternCnt = [{ type: /^(?=.*?[가-힣])/ }, { type: /^.{3,8}$/ }].filter((item) => {
...
const validatePasswordCheck = (password: string | undefined, passwordCheck: string | undefined) =>
  password !== passwordCheck ? '비밀번호가 확인값과 다릅니다. 다시입력 해주세요.' : null;
