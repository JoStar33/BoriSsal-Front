...
const UserDeliverAddressPart = ({ addressInfo, labelInfo, addressType }: IProps) => {
  ...
  const { isLoading, isError, isSuccess, error, mutate } = useDeliverAddressMutation(userAddressInfo);
  useEffect(() => {
  ...
  return (
    <>
      <div className={styles.user_deliver_address_container}>
        ...
        {!(addressType === 'address') ? (
          <input onChange={handleChangeAddress} ref={inputRef} type="text" role={addressType} />
        ) : (
          <p className={styles.address} role="address">
            {address}
          </p>
        )}
        {!(addressType === 'address') ? (
          <button aria-label="주소 수정 버튼" className={styles.modify_button} onClick={() => mutate()} role="modify_address_button">
            <BsFillPencilFill style={{ width: '1vw', height: '1vw' }} />
          </button>
        ) : (
          <button aria-label="주소 검색 버튼" className={styles.address_search_button} onClick={() => setDialog(true)}>
            주소검색
          </button>
        )}
        <>
