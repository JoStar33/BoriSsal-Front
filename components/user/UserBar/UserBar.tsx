    ...
    <>
      ...
      <div className={styles.userbar_container}>
        <Link href="/cart-page" aria-label="장바구니 페이지로 이동">
          <BsFillCartFill
            color="black"
            style={{ width: '3vw', height: '3vw', position: 'relative', minWidth: '1.5rem', minHeight: '1.5rem' }}
          />
        </Link>
        <Link className={styles.user_profile_container} href="/user" aria-label="사용자 페이지로 이동">
          {user.profile_image ? (
            <figure
              style={{ width: '3vw', height: '3vw', position: 'relative', borderRadius: '100px', minWidth: '1.5rem', minHeight: '1.5rem' }}
            >
              <Image
                role="profile"
            ...
            <div role="non_profile" className={styles.non_profile} />
          )}
        </Link>
        ...
        <button aria-label="로그아웃 버튼" className={styles.logout_button} onClick={() => mutate()}>
          Logout
        </button>
      </div>
    </>
