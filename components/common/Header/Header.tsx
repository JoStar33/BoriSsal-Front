...
const HeaderUserPart = dynamic(() => import('../HeaderUserPart/HeaderUserPart'), {
  ssr: false,
    ...
    <>
      <div className={styles.header}>
        <div className={styles.side_bar_button}>
          <AiOutlineMenu size={40} onClick={() => setShowSideBar(!showSideBar)} />
        </div>
        <div className={styles.logo_box}>
          <Link href="/" aria-label="메인페이지로 이동">
            <Image role="main-link" src={logo} alt="보리쌀 로고" width={50} height={50} />
          </Link>
        </div>
        <HeaderUserPart />
      </div>
