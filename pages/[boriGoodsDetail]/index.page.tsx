...
import { useBoriGoodsReplyQuery } from '@/hooks/bori-goods/useBoriGoodsReplyQuery';
...
import { useLikeGoodsMutation } from '@/hooks/bori-goods/useLikeGoodsMutation';
...
const BoriGoodsDetail = (
  ...
  const likeGoodsMutation = useLikeGoodsMutation(user.id, goods._id, user.user_product_like.find(likeGoods => likeGoods === goods._id));
  const handleLikeGoods = () => {
    if (!user.id) {
      validateText.current = '로그인 이후에 누를 수 있어요!';
      return setDialog(true);
    };
    user.user_product_like.find(likeGoods => likeGoods === goods._id) ? goods.product_like-- : goods.product_like++
    ...
    <div>
      ...
      <div className={styles.bori_goods_info_container}>
        <div className={styles.goods_image_container}>
          <figure style={{position: "relative", width: '48vw', height: '48vw'}}>
            <Image
              ...
              alt={goods.product_name}></Image>
          </figure>
          <div className={styles.button_container}>
            <button className={styles.cart_button}>
              <BsFillCartFill size={40}></BsFillCartFill>장바구니 담기</button>
        ...
        <div className={styles.goods_info}>
          ...
          <p className={styles.goods_like_container}>좋아요: 
            <div>
