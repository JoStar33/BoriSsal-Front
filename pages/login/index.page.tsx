...
const Login = () => {
  ...
  return (
    <>
      <NextSeo title="로그인" description="어서오세요! 로그인을 하시고 더 다양한 기능을 활용해보세요~" />
      {(loginMutation.isLoading || isLoading) && <Loading />}
      <div className={styles.login_container}>
        <h1>로그인</h1>
        <div className={styles.login_box}>
          <div className={styles.input_container}>
            <p>이메일:</p>
            <input role="email" type="text" name="email" onKeyDown={handleOnKeyDown} onChange={onChangeAccount} />
          </div>
          <div className={styles.validate_text}>{validateEmail(account.email)}</div>
          <div className={styles.input_container}>
            <p>비밀번호:</p>
            <input role="password" type="password" name="password" onKeyDown={handleOnKeyDown} onChange={onChangeAccount} />
          </div>
          <div className={styles.validate_text}>{validatePassword(account.password)}</div>
          <div>
            <button aria-label="로그인 버튼" className={styles.login_button} onClick={() => handleLogin()} role="login">
              로그인
            </button>
            <Link href="/join" aria-label="회원가입 페이지로 이동">
              <button aria-label="회원가입 버튼" className={styles.join_button}>
                회원가입
              </button>
            </Link>
          </div>
          <div style={{ fontSize: '0.8rem', marginBottom: '1rem', fontWeight: 800 }}>
            <Link href="/find-password" aria-label="비밀번호 찾기 페이지로 이동">
              😲비밀번호를 까먹었어요!
            </Link>
          </div>
        </div>
