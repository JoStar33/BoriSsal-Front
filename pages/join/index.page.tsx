    ...
    <>
      ...
      <div className={styles.join_container}>
        ...
        <div className={styles.join_container__part}>
          ...
          />
          <DuplicateCheckPart validate={validateEmail(account.email)} info={account.email} type={true} />
        </div>
        <div className={styles.join_container__part}>
          <InputPart
            validate={validateNick(account.nick)}
            inputLabel="🐶닉네임:"
            inputName="nick"
            textOrPassword="text"
            onChangeAccount={onChangeAccount}
          />
          <DuplicateCheckPart validate={validateNick(account.nick)} info={account.nick} type={false} />
        </div>
        <div className={styles.join_container__part}>
          <InputPart
            ...
            onChangeAccount={onChangeAccount}
          />
          <div style={{ width: '13vw', height: '1vw' }} />
        </div>
        <div className={styles.join_container__part}>
          <InputPart
            validate={validatePasswordCheck(account.password, account.passwordCheck)}
            inputLabel="🐶비밀번호 확인:"
