...
const OrderPage = () => {
  ...
  const totalPrice = useMemo(() => {
    return cart.reduce((_total, cartElement) => {
      return _total + cartElement.bori_goods_count * cartElement.bori_goods_price;
    }, 0);
  }, [cart]);
  ...
  return (
    <>
      <NextSeo title="주문하기" description="저희 보리의 굿즈를 주문하시는 페이지에요!" />
      {orderShow ? (
        <div className={styles.order_container}>
          ...
          <div className={styles.info_container}>
            <UserDeliverAddressPart addressInfo={deliverAddress.phone_number} addressType="phone_number" labelInfo="전화번호: " />
            <UserDeliverAddressPart addressInfo={deliverAddress.address} addressType="address" labelInfo="주소: " />
            <UserDeliverAddressPart addressInfo={deliverAddress.address_detail} addressType="address_detail" labelInfo="상세주소: " />
          </div>
          <h1 className={styles.info_head}>주문 상품 정보</h1>
          <div className={styles.cart_container}>
            {cart.map((cartElement) => (
              <CartItem cart_id={''} key={cartElement.bori_goods_id} cartGoods={cartElement} />
            ))}
          </div>
          ...
          <button aria-label="최종 주문하기 버튼" role="order-button" onClick={handleOrder} className={styles.order_button}>
            주문하기
          </button>
        </div>
      ) : (
