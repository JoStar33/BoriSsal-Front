      ...
      <div className={styles.userpage_container}>
        <div className={styles.user_image__deliver_info}>
            ...
            <div style={{position: 'relative'}}>
              {
                ...
                ? <div>
                    <Image
                      ...
                      width={200}
                      height={200}
                      src={process.env.NEXT_PUBLIC_BORI_SSAL_API_URL + user.profile_image}>
                    </Image>
                  </div>
                : <div className={styles.none_image}></div>
              }
              <label 
                style={{cursor: 'pointer', position: 'absolute', top: '150px', left: '150px'}} 
          ...
          <div className={styles.user_deliver_address}>
            <h1>배송정보 변경</h1>
            <UserDeliverAddressPart user_id={user.id} addressInfo={data?.data[0].phone_number} addressType='phone_number' labelInfo='전화번호: '></UserDeliverAddressPart>
            <UserDeliverAddressPart user_id={user.id} addressInfo={data?.data[0].address} addressType='address' labelInfo='주소: '></UserDeliverAddressPart>
            <UserDeliverAddressPart user_id={user.id} addressInfo={data?.data[0].address_detail} addressType='address_detail' labelInfo='상세주소: '></UserDeliverAddressPart>
          </div>
        </div>
