...
const BoriGoodsDetail = ({ goods, category, categoryErrorMessage, goodsErrorMessage }: IProps) => {
  ...
  if (goodsErrorMessage || categoryErrorMessage) {
    return <ErrorPage error={error} errorText={goodsErrorMessage || categoryErrorMessage} />;
  }

  return (
    <>
      <NextSeo title={`${goods.bori_goods_name}`} description={`${goods.bori_goods_desc}`} />
      <div className={styles.bori_goods_detail_container}>
        {/*굿즈의 상세정보 조회*/}
        <BoriGoodsDetailInfo goods={goods} category={category} user={user} />
        {isLoading && <ReplyLoading />}
        {!isLoading && (
          <ReplyViewer user={user} refetch={refetch} setLimit={setLimit} limit={limit} goods_id={goods._id} mutationData={boriGoodsReply} />
        )}
      </div>
    </>
  );
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const goods = goodsData.find((goods) => goods._id === params.boriGoodsDetail);
  if (!goods) {
    return {
      props: {
